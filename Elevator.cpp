...
void Elevator::setCurrentFloor(int f)
{
	...
	if (f == -1) { //if floor didn't change, because no new requests 
		...
		cout << "\nElevator " << IDNum << " went from floor [" << current_floor;
		current_floor = f;
...
Passenger Elevator::exit(int cf)
{
	for (int i = 0; i < elev_passengers.size(); i++) {
		if (elev_passengers[i].getDestination() == cf) { //looks at all passengers in the elevator
