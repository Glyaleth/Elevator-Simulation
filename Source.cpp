...
//***TODO*** BUILDING: once we get the #elevators we will make a vector of type elevators to keep track of n elevators 

//My_Building.add_Passenger(the_passenger, the_passenger.getDirection());
...
//***TODO*** ELEVATOR: Create a bool .StillExiting() that checks if there are still passengers that need to get "popped" out of the elevator 


//Passenger exit_passenger = My_Elevator.Exit();
//***TODO*** ELEVATOR: Create function to decide who needs to get off on the current floor it is on									  
//***TODO*** ELEVATOR: .Exit() should return the passenger that just exited the elevator


//while (!My_Elevator.StillLoading()) {//While there are still people that need to get into the elevator
//***TODO*** ELEVATOR: create bool StillLoading function that checks if there is still anyone on the current floor that needs to be loaded in

//My_Elevator.load(loading_passenger, loading_passenger.getDestination());
//***TODO*** ELEVATOR: Create Elevator function to load a passenger into the elevator that takes in the passenger getting loaded, and their destination, and returns void??
	...
	while (again) {
		...
		if (choice == 1) {
			...
			while (num_exited < p_num) { //while there are still passengers who haven't made it to their destination
				...
				//Chale: shouldn't Elevator class call move and not be within Building Decide function? Building's decide function should return just the floor it needs to go to
